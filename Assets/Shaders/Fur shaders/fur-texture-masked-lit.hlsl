#ifndef FUR_TEXTURE_MASKED_LIT_HLSL
#define FUR_TEXTURE_MASKED_LIT_HLSL
...
half4 _BlockFresnelColor;
float _BlockFresnelPower;
half _BlockFresnelVis;
...
float _NormalScale;
float _FurNormalScale;
TEXTURE2D(_FurNormals); 
SAMPLER(sampler_FurNormals);
...
float3 Whiteout(float3 n1, float3 n2) {
...
float4 frag(v2f input) : SV_Target {
  ...
  float alpha = furColor.r * (1.0 - input.layer) * maskMap.a;
  if (input.layer > 0.0 && alpha < _AlphaThreshold) discard;

  float3 viewDirWS = SafeNormalize(GetCameraPositionWS() - input.positionWS);
  float3 normalFurTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_FurNormals, sampler_FurNormals, furUV), _FurNormalScale);
  float3 normalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, input.uv), _NormalScale);
  float3 combinedNormals = Whiteout(normalTS, normalFurTS);
  float3 bitangent = SafeNormalize(-cross(input.normalWS, input.tangentWS));
  float3 normalWS = SafeNormalize(TransformTangentToWorld(combinedNormals, float3x3(input.tangentWS, bitangent, input.normalWS)));

  SurfaceData surfaceData = (SurfaceData)0;
  InitializeStandardLitSurfaceData(input.uv, surfaceData);
  float3 masks = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.uv);
  surfaceData.albedo = lerp(lerp(lerp(float3(0,0,0), _ColorMain, masks.r), _ColorHigh, masks.g), _ColorDarker, masks.b);
  ...
  color.rgb = lerp(color.rgb, _BlockFresnelColor, saturate(pow(1-abs(dot(viewDirWS, input.normalWS)), _BlockFresnelPower)*_BlockFresnelVis));
